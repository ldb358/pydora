...
class Ui(object):
    ...
    def __init__(self):
        ...
        self.__pan.connect();
        #initalize the pydora web interface
        self.__web = pydora_web.Pydora_Web();
        #initilize gui
        root = Tk();
        root.resizable(0, 0);
        self.root = root;
    ...
    def button_callback(self, name):
        ...
        try:
            ...
            webpass = textboxes['webpass'].get().strip();
            webtid = textboxes['webtid'].get().strip();
            self.user_login(user, password, webpass, webtid);
        except:
            if name == "pause":
                self.__media.pause();
                self.Gui.set_buttons(self.button_callback);
    ...
    def user_login(self, user, password, webpassword=False, token=False):
        try:
            self.__pan.auth(user, password, webpassword);
            if webpassword:
                self.__web.auth(user, webpassword);
                self.__web.add_device(token);
        except:
    ...
    def time(self):
        ...
        if self.__web.enabled():
            commands = self.__web.get_commands();
            if commands != 0:
                for x in commands:
                    self.do_command(x);
